...
def reset(event):
    ...
    while len(rectangle_group) < 15:#This number changes what the minimum rectangles that can be in a group
        ...
        rx1 = random.randint(30,width)
        rx0 = rx1-30
        ry1 = random.randint(30, height )
        ry0 = ry1-30
        rectangle= blTr([rx0,ry0,rx1,ry1])
        rectangle_group = []
    ...
    draw.rectangle(re_orient(rectangle_group[0]), fill = "red")
    
    for i in range(len(rectangle_group)-1):      
        canvas.create_rectangle(rectangle_group[i])
        
        temp = re_orient(rectangle_group[i])
        draw.rectangle(temp, outline = "black")
    ...
    draw.rectangle(re_orient(rectangle_group[-1]), fill = "green")
